...
// AUTHOR:  0b5vr
// FOUND:   https://www.shadertoy.com/view/cs2GWD
...
vec4 map(vec3 p,Grid grid){
  p-=grid.c;
  p.z+=.4*sin(2.*SPEED+1.*fract(grid.h.z*28.)+.3*(grid.c.x+grid.c.y));
  ...
  if(grid.i==0){//2x2
    if(grid.h.y<.3){//speaker
      ...
      pt.y-=.5-smoothstep(-.5,.5,sin(SPEED+c.x+grid.h.z*100.));
      ...
      float ani=smoothstep(-.5,.5,sin(SPEED+grid.h.z*100.));
      ...
      if(d3<d){
        float led=1.-ani;
        led*=.5+.5*sin(SPEED*exp2(3.+3.*grid.h.z));
    ...
    if(grid.h.y<.4){//fader
      ...
      float ani=smoothstep(-.2,.2,sin(SPEED+grid.h.z*100.));
      pt.x-=mix(-.8,.8,ani);
      ...
      float ani=smoothstep(.8,.9,sin(10.*SPEED-c.x*2.2+grid.h.z*100.));
    ...
    if(grid.h.y<.5){//knob
      ...
      float ani=smoothstep(-.5,.5,sin(2.*SPEED+grid.h.z*100.));
      pt.xy*=r2d(PI/6.*5.*mix(-1.,1.,ani));
      ...
      float ani=sin(2.*SPEED+grid.h.z*100.);
...
void main ()
{
  vec2 fragCoord = gl_FragCoord.xy * vec2(1.0, -1.0);
  ...
  float canim=smoothstep(-.2,.2,sin(SPEED));
  ...
  if(march.isect.x<1E-2){
    ...
    if(mtl==0.){
      ...
      led*=vec3(2,1,2);
      float plasma=sin(length(c)*10.-10.*SPEED+march.grid.h.z*.7);
      plasma+=sin(c.y*10.-7.*SPEED);
      led*=.5+.5*sin(plasma);
      col+=2.*led;
      basecol=.04*led;
    }else if(mtl==5.){//808
